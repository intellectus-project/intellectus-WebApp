...
export const compare = (element1, element2, key) => {
  ...
  return JSON.stringify(element1[key]).localeCompare(
    JSON.stringify(element2[key])
  );
};
...
export const parseToFilterDropDown = (entries, key, title, childs = null) => {
  return entries.map(function (entry) {
    ...
    if (
      childs &&
      entry.hasOwnProperty(childs.data) &&
      entry[childs.data].length > 0
    ) {
...
export const diffForHumans = (date) => {
  let formattedDate = date.replace("T", " ").split(".")[0];
  let before = moment
    .utc(formattedDate, "YYYY.MM.DD HH:mm:ssZ")
    .clone()
    .tz(moment.tz.guess());
